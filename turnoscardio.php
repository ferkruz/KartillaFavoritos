...
<li>
<span class='name'>Dr. Diego Villaba</span><br><span class='tel'><i class='mdi mdi-hospital'></i>
<span class='tel'>Prevención cardiologica</i>
</li>

<li>
<span class='name'>Dra. Karina Alonso</span><br><span class='tel'><i class='mdi mdi-hospital'></i>
<span class='tel'>Prevención cardiologica</i>
</li>

<li>
<span class='name'>Dra. Karina Sánchez</span><br><span class='tel'><i class='mdi mdi-hospital'></i>
<span class='tel'>Prevención cardiologica</i>
</li>

<li>
<span class='name'>Dra. Verónica Yánez</span><br><span class='tel'><i class='mdi mdi-hospital'></i>
<span class='tel'>Prevención cardiologica</i>
</li>
    </ul> 
  ...
  <!-- <script type="text/javascript">
   ...
   $SQLconsulta_cartilla="SELECT * FROM tbl_turnospprev WHERE afi=$nom and tipodok=$tipodoku";
   $consulta_cartilla = mysql_query($SQLconsulta_cartilla,$connec) or die(mysql_error());
  ...
  do {
   ...
   if (($registro_cartilla['cita']) <= $hoy){
	...
	$presttem = $registro_cartilla['prest'];
 	$updateSQL = sprintf("UPDATE tbl_turnospprev SET cita=%s WHERE prest=%s and afi=%s and tipodok=%s",	
  ...
  //echo 'konsult'.$updateSQL;
		...
		?>
            ...
            </span><span class='phone'><i class='mdi mdi-hospital'></i><?php echo utf8_encode($registro_cartilla['pprev']) ?>
...
?> 
  ...
  <!--  <ul class="list mat-ripple">      
    ...
    <div class="list-chat">
      ...
      <i class="mdi mdi-account-multiple"></i><span class='name'> Prevención cardiologica</span>
      </li>
      <!--<li data-route=".list-text">
      <i class="mdi mdi-comment-text"></i>
      </li>-->
      <li data-route=".list-phone">
      <i class="mdi mdi-calendar"></i><span class='name'> Mis Turnos</span></li></ul>
    </div> 
  ...
  <div id="contact-modal" data-mode="add" class="card dialog">
    <h3>Add Contact</h3>
      ...
      <script type="text/javascript">
    		function editpres(){
			 ...
			 var salv=""+"../editfav.php?pres="+doc+"&regi="+fav+"&afi="+ <?php echo $nom ?> +"&tip="+ <?php echo $tipodoku ?>+"&regi="+"Prevención cardiologica";    			
				window.open(salv, "_self");
   			}
    </script>
      </form>
    ...
    <div class="btn-container">
      ...
      <script type="text/javascript">
    		function borroturno(){
			 ...
			 //var salv=""+"../editfav.php?pres="+doc+"&regi="+fav+"";
			 var borro=""+"../borroturnopprev.php?pres="+doc+"&cita="+cita+"&afi="+ <?php echo $nom ?> +"&tip="+ <?php echo $tipodoku ?>;    			
				window.open(borro, "_self");
   			}
    </script>
      </form>
    ...
    <script type="text/javascript">
    		function cita(){
			 ...
			 //var fav=document.getElementById("new-user0").value;
			var salvcita=""+"../adddatepprev.php?pres="+dock+"&cita="+date+"&afi="+ <?php echo $nom ?> +"&tip="+ <?php echo $tipodoku ?>+"&prog=Prevención cardiologica";
...
switch (docky) { 
	case 'Dr. Diego Villaba': 

		
	if ((currentDateTime.getDay() == 1)||(currentDateTime.getDay() == 2)){
		<?php	
	mysql_select_db($database_connec, $connec);
	$query_turnos = "SELECT * FROM tbl_turnospprev WHERE prest='Dr. Diego Villaba'";
	$turnos = mysql_query($query_turnos, $connec) or die(mysql_error());
	$row_turnos = mysql_fetch_assoc($turnos); 
	$totalRows_turnos = mysql_num_rows($turnos);
	

$horas=array ("'10:30'","'10:50'","'11:10'","'11:30'","'11:50'","'12:10'");		
...
<?php $kalendartotal="['10:30','10:50','11:10','11:30','11:50','12:10']";?>
...
if ((diaokupa == <?php echo $diaokupa ?> )&&(mesokupa == <?php echo $mesokupa ?>)&&(anhookupa == <?php echo $anhookupa ?>)){
			...
			if ($index!==false)
			array_splice($horas, $index, 1);
			$kalendar="[".$horas[0].",".$horas[1].",".$horas[2].",".$horas[3].",".$horas[4].",".$horas[5]."]"; 
			?>
...
break;
	case 'Dra. Karina Sánchez': 

		
	if ((currentDateTime.getDay() == 4)){
		<?php	
    $torda='Dra. Karina Sánchez';
	mysql_select_db($database_connec, $connec);
	$query_turnos = "SELECT * FROM tbl_turnospprev WHERE prest='".utf8_decode($torda)."'";
	$turnos = mysql_query($query_turnos, $connec) or die(mysql_error());
	$row_turnos = mysql_fetch_assoc($turnos); 
	$totalRows_turnos = mysql_num_rows($turnos);
...
break;
	case 'Dra. Karina Alonso': 

		
	if ((currentDateTime.getDay() == 3)){
		<?php	
	mysql_select_db($database_connec, $connec);
	$query_turnos = "SELECT * FROM tbl_turnospprev WHERE prest='Dra. Karina Alonso'";
	$turnos = mysql_query($query_turnos, $connec) or die(mysql_error());
	$row_turnos = mysql_fetch_assoc($turnos); 
	$totalRows_turnos = mysql_num_rows($turnos);
...
break;
	case 'Dra. Verónica Yánez': 

		
	if ((currentDateTime.getDay() == 5)){
		<?php	
	$torda1='Dra. Verónica Yánez';
	mysql_select_db($database_connec, $connec);
	$query_turnos = "SELECT * FROM tbl_turnospprev WHERE prest='".utf8_decode($torda1)."'";
	$turnos = mysql_query($query_turnos, $connec) or die(mysql_error());
	$row_turnos = mysql_fetch_assoc($turnos); 
	$totalRows_turnos = mysql_num_rows($turnos);
