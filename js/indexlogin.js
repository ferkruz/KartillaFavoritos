    ...
    if (localStorage.getItem('color') !== null) {
        ...
        var colorarray = [0, 204, 255]; // 15 157 88 = #0f9d58
    ...
    function setName(name) {
        //$.trim(name) === '' || $.trim(name) === null ? name = 'Su Cartilla' : name = name;
        //$('h1').text(name);
    ...
    function setModal(mode, $ctx, $ctx0) {
        var $mod = $('#contact-modal');
		var $mod0 = $('#calendar-modal');
		var $mod1 = $('#delete-modal');
		//var $mod2 = $('#email-modal');
        switch (mode) {
            /*case 'add':
                $mod.find('h3').text('Añadir Contacto');
                break;*/

            case 'edit':
                $mod.find('h3').text('Editar Contacto');                
				$mod.find('#new-user').val($ctx.text()).addClass('used');
				$mod.find('#new-user0').val($ctx.text()).addClass('used');
                break;
				
		   case 'calendar':
                $mod0.find('h3').text('Añadir turno');
				$mod0.find('#user').val($ctx.text()).addClass('used');
				$mod0.find('#user0').val($ctx.text()).addClass('used');
				$mod0.find('#new-date').val($ctx.text()).addClass('date');
                //$mod.find('#new-user').val($ctx.text()).addClass('used');
				//$mod.find('#new-user0').val($ctx.text()).addClass('used0');*/
				$mod0.fadeIn();
        		$('.overlay').addClass('date');
        		$mod.find('#new-date').focus();
                break;
				
		   case 'delete':
                $mod1.find('h3').text('Borrar Contacto');
				$mod1.find('#userb').val($ctx.text()).addClass('used');
				$mod1.find('#userb0').val($ctx.text()).addClass('used');
                $mod1.fadeIn();
        		$('.overlay').addClass('borro');
        		$mod1.find('#new-user').focus();
                break;				
    ...
    $('#username').on('blur', function() {
        ...
        $('.cardlogin.menu > .header > img').addClass('excite');
        setTimeout(function() {
            $('.cardlogin.menu > .header > img').removeClass('excite');
        }, 800);
    ...
    $('.list-account > .list').on('click', 'li> span.name', function() {
        $(this).parent().children().removeClass('active');
        $(this).parent().find('.context').remove();
        $(this).addClass('active');
        //var $TARGET = $(this, 'li> span>');
		var $TARGET = $(this, '.list-account > .list mat-ripple> li> span> .name');
		//var $TARGET = $('.list-account > .list mat-ripple> li> span.name'); 
        if (!$(this).next().hasClass('context')) {
            var $ctx = $('<li class="context"><i class="mdi mdi-pencil"></i><i class="mdi mdi-delete"></i><i class="mdi mdi-calendar"></i></li>');
            ...
            $ctx.on('click', '.mdi-pencil', function() {
                ...
                $('#contact-modal').one('click', '.btn.save', function() {
					...
					//$TARGET.find('.name').text($('#new-user0').val());
                    closeModal();
                });
            });
			...
			$ctx.on('click', '.mdi-calendar', function() {
                setModal('calendar', $TARGET);

               /* $('#calendar-modal').one('click', '.btn.save', function() {
                    //$TARGET.find('.name').text($('#new-user').val());
					//$TARGET.find('.name').text($('#new-user0').val());
                    closeModal();
                });*/
	...
	/* $('#head').on('click', '.mdi-email', function() {
		 alert ("emailooo");
		  // var $ctx0 = $('#head');
		    //var $ctx0= $('#head');
			var $ctx0 = $('#hangout .mdi-email');
			/*$ctx0.on('click', '.mdi-email', function() {
				alert ("789email00000000");
			});*/
			  
			   /*var $mod2 = $('#email-modal');
		  		alert ("123"+ $mod2);
		 		$mod2.find('h3').text('Añadir email');
				$mod2.find('#mail').val($ctx0.text()).addClass('used');
				$mod2.find('#mail0').val($ctx0.text()).addClass('used');
				alert ("123"+$mod2+$ctx0.text());
                $mod2.fadeIn();
        		$('.overlay').addClass('mail');
        		$mod2.find('#new-mail').focus();
                setModal('addemail', $TARGET);

                /*$('#mail-modal').one('click', '.btn.save', function() {
                    $TARGET.find('.name').text($('#new-mail').val());
					//$TARGET.find('.name').text($('#new-user0').val());
                    closeModal();                
            });
	...
	$('#content .btn-container').on('click', function() {
        $('.menu').toggleClass('open');
        $('.overlay').toggleClass('add');
    });
	...
	$('#calendar-modal').on('click', '.btn.cancel', function() {
        closeModal();
    });
	...
	$('#new-date').on('keydown', function(event) {
        switch (event.which) {
            case 13:
                event.preventDefault();
                $('.btn.save').trigger('click');
